...
exports.regUser = (req,res)=>{
    ...
    db.query(sqlStr,userinfo.username,(err,results)=>{
        ...
        db.query(sql,{username: userinfo.username , password: userinfo.userpassword},(err,results)=>{
            // 执行 SQL 语句失败
            if(err){
                // *return res.send({ status: 1,message: err.message })
                return res.cc(err)
...
exports.login = (req,res)=>{
    ...
    db.query(sql,userinfo.username,(err,results)=>{
        ...
        // 判断密码
        // 拿着用户输入的密码，和数据库的密码
        const compareResult = bcrypt.compareSync(userinfo.userpassword,results[0].password)
